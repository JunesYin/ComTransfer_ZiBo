...

{
	...
	*/
	...
	LPCWSTR szPort_tmp = charToWchar(szPort);

	m_hComm = CreateFile(szPort_tmp,						// communication port string (COMX)
	...
	if (m_hComm == INVALID_HANDLE_VALUE)
	{
		...
		case ERROR_FILE_NOT_FOUND:
			{
				CString str;
				str.Format(_T("COM%d ERROR_FILE_NOT_FOUND,Error Code:%d"),portnr,GetLastError());
				AfxMessageBox(str);
				break;
			}
			//串口拒绝访问
		case ERROR_ACCESS_DENIED:
			{
				CString str;
				str.Format(_T("COM%d ERROR_ACCESS_DENIED,Error Code:%d"),portnr,GetLastError());
				AfxMessageBox(str);
				break;
			}
		default:
			...
			case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
				{
					...
					CString str;
					str.Format(_T("COM%d ERROR_ACCESS_DENIED，WaitCommEvent() Error Code:%d"),port->m_nPortNr,GetLastError());
					AfxMessageBox(str);
					break;
				}
			...
			case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
				{
					port->m_hComm = INVALID_HANDLE_VALUE;
					CString str;
					str.Format(_T("COM%d ERROR_BAD_COMMAND，WaitCommEvent() Error Code:%d"),port->m_nPortNr,GetLastError());
					AfxMessageBox(str);
					break;
				}
...
void CSerialPort::ProcessErrorMessage(char* ErrorText)
{
	...
	WCHAR *tmp_tmp = charToWchar(Temp);
	MessageBox(NULL, tmp_tmp, _T("Application Error"), MB_ICONSTOP);
			...
			case ERROR_ACCESS_DENIED:///拒绝访问 erroe code:5
				{
					port->m_hComm = INVALID_HANDLE_VALUE;
					CString str;
					str.Format(_T("COM%d ERROR_ACCESS_DENIED，WriteFile() Error Code:%d"),port->m_nPortNr,GetLastError());
					AfxMessageBox(str);
					break;
				}
			case ERROR_INVALID_HANDLE:///打开串口失败 erroe code:6
			...
			case ERROR_BAD_COMMAND:///连接过程中非法断开 erroe code:22
				{
					port->m_hComm = INVALID_HANDLE_VALUE;
					CString str;
					str.Format(_T("COM%d ERROR_BAD_COMMAND，WriteFile() Error Code:%d"),port->m_nPortNr,GetLastError());
					AfxMessageBox(str);
					break;
				}
			default:
...
void CSerialPort::ReceiveStr(CSerialPort* port)
{
	...
	{ 
		...
		//WCHAR *RXBuff = new WCHAR[comstat.cbInQue+1];	//modify by junes
		if(RXBuff == NULL)
		{
			return;
		}
...
void CSerialPort::WriteToPort(LPCTSTR string)
{
	...
	strcpy_s(m_szWriteBuffer,m_nWriteBufferSize,str);
	m_nWriteSize=strlen(str);
	// set event for write
	SetEvent(m_hWriteEvent);
}
